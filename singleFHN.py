...
def FHN(N,dt,T,a,b,c,ar,ad,Tmax,Vrev,la,VT,V0,y0,w0,Iprev,J,sigex,sigJ,det,det2):
    ...
    noise=[] #list of noise realizations
    ...
    for r in range(1,M):        
        ...
        for i in range(N):
            ...
            if z[i][2,r-1] < 1 and z[i][2,r-1] > 0 and det==0:
                #o[2,1]=sig2
                o[2,1]=(0.1*np.exp(-0.5/float(1-np.power(2*z[i][2,r-1]-1,2))))*np.sqrt(ar*(Tmax/float(1+np.exp(-la*(z[i][0,r-1]-VT))))*(1-z[i][2,r-1])+ad*z[i][2,r-1])
